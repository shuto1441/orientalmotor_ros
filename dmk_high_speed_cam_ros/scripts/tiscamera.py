...
class Camera:
    ...
    def __init__(self, serial, width, height, framerate, color, liveview, topic_name=None, node_name=None):
        ...
        gscam = 'shmsrc socket-path=/tmp/ros_mem ! video/x-raw, format=GRAY8,width=%d,height=%d,framerate=%d/1 ! videoconvert' % (
            width, height, framerate,)
        #gscam += ',bpp=8,depth=8,blue_mask=16711680, green_mask=65280, red_mask=255 ! ffmpegcolorspace'
