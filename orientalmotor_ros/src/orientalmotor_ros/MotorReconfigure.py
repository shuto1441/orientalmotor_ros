...
class OrientalMotor:
    def __init__(self, port):
        ...
        rospy.Subscriber(
            "motor/positioning_rotate",
            Empty,
            self.positioning_rotate)
        rospy.Subscriber(
            "motor/continuous_rotate",
            Empty,
            self.continuous_rotate)
        rospy.Subscriber("motor/stop", Empty, self.off)

    def callback(self, config, level):
        rospy.loginfo(
            """Reconfigure Request: {rpm}, {angle}, {reverse}""".format(
                **config))
    ...
    def _apply_acceleration(self, acceleration):
        command = b"\x01\x06\x06\x01" + \
            self._rpm_acceleration_to_bytes(acceleration)
    ...
    def _apply_deceleration(self, deceleration):
        command = b"\x01\x06\x06\x81" + \
            self._rpm_acceleration_to_bytes(deceleration)
        command += self._error_check(command)
        self.client.write(command)
        result = self.client.read(self.size)
    ...
    def _apply_angle(self, msg):
        ...
        print(reverse)
        if(reverse):
            command = b"\x01\x10\x04\x00\x00\x02\x04" + \
                self._angle_to_bytes_rvs(angle)
        else:
            command = b"\x01\x10\x04\x00\x00\x02\x04" + \
                self._angle_to_bytes(angle)
        command += self._error_check(command)
        self.client.write(command)
        result = self.client.read(self.size)
    ...
    def _apply_rpm(self, rpm):
        command = b"\x01\x10\x04\x80\x00\x02\x04\x00\x00" + \
            self._rpm_to_bytes(rpm)
        command += self._error_check(command)
        self.client.write(command)
        result = self.client.read(self.size)
